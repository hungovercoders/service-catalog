"""MCP server exposing a service catalog: contracts, specs, features, docs.

Design notes:

* The catalog is a graph of services, not a flat list of files. Services own
...
* Every read is confined to CATALOG_DIR and to paths a service manifest
...
def catalog_dir() -> Path:
    raw = os.environ.get("CATALOG_DIR")
    if not raw:
        raise RuntimeError(
            "CATALOG_DIR is not set. In a plugin this is wired up in "
            ".mcp.json as ${CLAUDE_PLUGIN_ROOT}/catalog."
        )
    root = Path(raw).resolve()
    if not root.is_dir():
        raise RuntimeError(f"CATALOG_DIR does not exist: {root}")
    return root
...
def list_services() -> list[dict]:
    """List every service in the catalog with its domain, owner and summary.
...
@mcp.tool
def search_catalog(query: str, kind: str | None = None) -> list[dict]:
