"""Microcks mock-stack orchestration, driven entirely by the catalog.
...
def compose_file(arg: str | None) -> Path:
    ...
    target = Path(".catalog-kit/docker-compose.yml")
    target.parent.mkdir(exist_ok=True)
    target.write_bytes(
        (resources.files("catalog_kit") / "data/docker-compose.yml").read_bytes()
    )
    return target
...
def service_dirs(catalog_dir: str, only: str | None) -> list[Path]:
    dirs = [
        p.parent
        for p in sorted(Path(catalog_dir).glob("*/service.yaml"))
        if not only or p.parent.name == only
    ]
    if not dirs:
        raise SystemExit(f"no services matching '{only or '*'}' under {catalog_dir}/")
    return dirs
...
def run_test(
    ...
    if status >= 300 or not (test_id := json.loads(out or b"{}").get("id")):
        raise SystemExit(
            f"could not start the test - is '{service_id}' loaded? (catalog mocks load)"
        )
