...
so `catalog docs diagrams` gates them identically). Raw artifacts are
...
def http_operations(m: dict, catalog: Path, rest_examples: dict) -> list[dict]:
...
def changelog_data(m: dict) -> list[dict]:
    ...
    def commits(rev_range: str) -> list[str]:
        ...
        return subjects or [f"no commits under catalog/{name}/ in this release"]
...
def build_data(manifests: list[dict], catalog: Path, mocks: Path) -> dict:
    ...
    for m in manifests:
        ...
        ops = http_operations(m, catalog, rest_examples)
...
def collect_mermaid(data: dict) -> list[tuple[str, str]]:
    ...
    input `catalog docs diagrams` validates on an Astro-site repo."""
...
def run(catalog_dir: str, site_dir: str, mocks_dir: str = "mocks") -> int:
    ...
    data = build_data(manifests, catalog, Path(mocks_dir))
