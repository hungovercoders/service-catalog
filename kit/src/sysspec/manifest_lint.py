...
def run(only: str | None, specs_dir: str) -> int:
    specs = Path(specs_dir)
    service_dirs = sorted(p.parent for p in specs.glob("*/service.yaml"))
    if not service_dirs:
        print(
    ...
    if problems:
        ...
        return 1
    print(f"{checked} manifest(s) consistent with contracts and specs graph.")
    return 0
